...
> **Status:** Phase 2 implemented the application foundation described below: FastAPI backend, PostgreSQL/PostGIS data layer, Alembic migrations, RBAC, auth with rotating refresh tokens, and a typed Next.js frontend shell. Agent, provider, and orchestrator layers remain planned (Q2/Q3).
...
```
┌──────────────────────────────────────────────────────────────┐
│ Frontend  (implemented Phase 2: Next.js 15 / TS / Tailwind)   │
│  App shell, login, dashboard; map/timelines/chat land later   │
│  /api/* proxied to backend via next.config rewrites           │
└──────────────────────────────────────────────────────────────┘
                             │  HTTPS / JSON
┌──────────────────────────────────────────────────────────────┐
│ Backend API  (implemented Phase 2: Python 3.12 / FastAPI)     │
│  /api/v1: auth, users, organizations, workspaces,             │
│  saved-locations, health · Pydantic validation · RBAC         │
│  unified error contract {"error":{code,message,details}}      │
└──────────────────────────────────────────────────────────────┘
                             │
┌──────────────────────────────────────────────────────────────┐
...
│  PLANNED (Q2/Q3) — Satellite · Weather · Geocoding ·         │
│  Geospatial processing (indexes, masks, reprojection)         │
└──────────────────────────────────────────────────────────────┘
                             │
┌──────────────────────────────────────────────────────────────┐
...
| **Geocoding service** | Convert place names to coordinates; provider-independent. |
| **Geospatial processing** | Preprocessing, cloud masking, index computation (NDVI/NDWI), alignment, masking, derived features. |
...
- Phase 3 (Q2): agents + change engine.
- Phase 3 (Q3): orchestrator, LLM reasoning, suggestions.
...
- **Quality:** 21 backend tests (per-test disposable PostGIS DB), ruff lint/format clean, `tsc --noEmit` + eslint clean, `next build` green, backend Docker image built and smoke-tested.